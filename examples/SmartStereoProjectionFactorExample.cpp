...
int main(int argc, char **argv) {
  ...
  noiseModel::ExpandingIsotropic<3>::shared_ptr model(
      new noiseModel::ExpandingIsotropic<3>()); // 3D isotropic noise
  ...
  while (factor_file >> x >> l >> uL >> uR >> v >> _X >> Y >> Z) {
    ...
    cameras.emplace_back(Pose3(initial_estimate.at<Pose3>(X(x))),
                         K); // add camera to set
    factor->add(StereoPoint2(uL, uR, v), X(x), K);
    auto smart_noise =
        boost::dynamic_pointer_cast<noiseModel::ExpandingIsotropic<3>>(
            factor->noiseModel());
    if (smart_noise) {
      // add landmark measurement noise to the smart factor
      smart_noise->pushSigma(1);
    }
  }
  ...
  // during optimization
  //  NOTE: NonlinearEquality forces the optimizer to use QR rather than
  //  Cholesky QR is much slower than Cholesky, but numerically more stable
  ...
  if (output_poses) {
    ...
    for (size_t i = 1; i <= pose_values.size(); i++) {
      pose3Out << i << " "
               << pose_values.at<Pose3>(X(i)).matrix().format(
                      Eigen::IOFormat(Eigen::StreamPrecision, 0, " ", " "))
               << endl;
    }
  ...
  for (auto factor : graph) {
    auto smart_factor = boost::dynamic_pointer_cast<SmartFactor>(factor);
    if (smart_factor) {
      // compute hessian
      CameraSet<StereoCamera> factor_cameras = smart_factor->cameras(result);
      smart_factor->createHessianFactor(factor_cameras);
    }
  }
